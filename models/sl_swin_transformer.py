...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # padding
        # pad is the H and W of the input image are not multiples of patch_size
...
class ShiftedPatchTokenization(nn.Module):
    def __init__(
        self, patch_size=2, in_c=3, embed_dim=96, exist_class_t=False, is_pe=True
    ):
        super().__init__()
        self.patch_size = patch_size
        self.exist_class_t = exist_class_t
        self.patch_shifting = PatchShifting(patch_size)
        patch_dim = (in_c * 5) * (patch_size**2)
        if exist_class_t:
            self.class_linear = nn.Linear(in_c, embed_dim)
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        lsa_mask = torch.eye(n, n)
        self.lsa_mask = torch.nonzero((lsa_mask == 1), as_tuple=False)
    ...
    def forward(self, x, mask):
        ...
        dots = torch.mul(
            ...
            .unsqueeze(-1)
            .unsqueeze(-1)
            .expand((B_, self.num_heads, 1, 1)),
        )
        # lsa_mask = torch.eye(N, N)
        # lsa_mask = torch.nonzero((lsa_mask == 1), as_tuple=False)
        dots[:, :, self.lsa_mask[:, 0], self.lsa_mask[:, 1]] = -987654321
        attn = dots
...
class SLSwinTransformer(nn.Module):
    r"""Swin Transformer
    ...
    def __init__(
        ...
        **kwargs
        ...
        # stage4 outputs the matrix channels
